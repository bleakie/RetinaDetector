...
class RetinaFace:
    def __init__(self, gpu=0, vote=False):
        self.ctx_id = gpu
        self.network = 'net3'
        self.decay4 = 0.5
        ...
        self.threshold = config.TEST.SCORE_THRESH
        ...
        sym, arg_params, aux_params = mx.model.load_checkpoint(base_path + '/model/defect-v1.0/R50', 0)
        if self.ctx_id >= 0:
    ...
    def get_boxes(self, im, scales_index=2):
        ...
        if scales_index == 0:
            ...
            if im_shape[0] < target_h and im_shape[1] < target_w:
                scales = [1.0]
            else:
                scales = [im_scale]
        return scales

    def detect(self, img, scales_index = 2, do_flip=False):
        proposals_list = []
        scores_list = []
        landmarks_list = []
        ...
        for i in range(len(proposals)):
            if self.SaveRect(img, proposals[i]):
                index.append(i)
        if len(index) > 0:
    ...
    def detect_center(self, img, scales_index=2, do_flip=False):
        det, landmarks = self.detect(img, scales_index=scales_index)
        if det.shape[0] == 0:
            return None, None
        bindex = 0
        if det.shape[0] > 1:
